...
/**
 * Uninstalls an Aspen application.
 * For use on the command line only.
 *
 * @author P. Mark Anderson
 **/
public class RunAppUninstaller {

	public static void main(String[] args) {

		if(args.length != 1) {
			System.err.println("\n\nUsage:  RunAppUninstaller <install properties file>");
		...
		try { 
			...
			consoleDbPool = new DbConnectionPool(consoleURL, user, pwd, driver, 1, 1);

			String csDsn = props.getProperty("aspen.db.cs_dsn");
			String csURL = dbURL.toString() + csDsn;
			csDbPool = new DbConnectionPool(csURL, user, pwd, driver, 1, 1);

			// Turn off auto-commit so we can roll back if necessary
			consoleDbPool.getConnection().getConnection().setAutoCommit(false);
			csDbPool.getConnection().getConnection().setAutoCommit(false);
		...
		if (success) {
			try {
				...
				lw.logMsg("Run:","Creating new AppInstaller for app: " + 
						appDisplayName);
				AppInstaller ai = new AppInstaller(props, consoleDbPool, csDbPool, lw);

				lw.logMsg("Run:","\n\nINSTALLING APP\n");
				ai.install();

				lw.logMsg("Run:","\n\nCOMMITTING CHANGES TO DATABASES\n");
				csDbPool.getConnection().getConnection().commit();
				consoleDbPool.getConnection().getConnection().commit();
