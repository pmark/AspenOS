...
class MyTemplate {
	...
	/**
	 * Constructor. Allocates <code>StringBuffer</code> 
	 * for internal use.
	 */
	public MyTemplate(File file) {
		mFile = file;
	}
	 ...
	 */
	private int processBlock(OutputStream out, Map hash, MyBlock block) 
		throws IOException {
	 ...
	 */
	private int processNullBlock(OutputStream out, MyBlock block) {
	 ...
	 */
	private int processVarkeyBlock(OutputStream out, MyBlock block) 
		throws IOException {
	 ...
	 */
	private int processNormalBlock(OutputStream out, Map hash, MyBlock block)
		throws IOException {
		...
		while(ind < finalIndex) {
			...
			if(c == '$') {
				...
				MyBlock nextBlock = getBlock(ind);
	 ...
	 * of the object. 
	 */
	private int processVarBlock(OutputStream out, Map hash, MyBlock block)
		throws IOException {
	...
	private int processIfBlock(OutputStream out, Map hash, int start)
		...
		MyBlock block = getBlock(index + 1);
	 ...
	 */
	private boolean isTrueCondition(Map hash, MyBlock block) 
		throws IOException {
	...
	private List getForList(Map hash, 
								StringBuffer sb,
								MyBlock block) throws IOException {
	 ...
	 */
	private MyBlock getBlock(int start) {
		...
		return new MyBlock(mbyContent, start, index - start - 1);
	}
	...
	public void loadFile(OutputStream out, Map hash) 
		...
		try {	
			readFile();
			MyBlock block = new MyBlock(mbyContent);	
			processNormalBlock(out, hash, block);
		}
...
MyBlock {
	...
	MyBlock(byte[] by, int start, int length) {
		miStart = start;
		miLength = length;
		mbyArray = by;
	}
